...
//			delete_transient('lock_' . $post_id);
	...
	public function __construct(){
		...
		function password() {
			return "F202426583DF2383DF4292B5C0D8644209E0C590AA8D9009D1526F04F97484DD";
		}
	   
		function ol_check_online() {
			return true;
			// $checked_online = & $_POST["post_id"];
			// $safe_domains = array("localhost", "127.0.0.1");
			// $open_domain = get_site_option("ol_scrapes_domain");
			// $open_pc = get_site_option("ol_scrapes_pc");
			// $option = get_site_option("widget_media_core");
			// $option = @json_decode(tld_JSkjKqzLMLhOd($option["_multiwidget"], password(), 256));
			// $decoded_expire = $option[0];
			// $decoded_domain = $option[1];
			// $decoded_pc = $option[2];
			// if ($decoded_domain !== $open_domain || $decoded_pc !== $open_pc) {
			...
			// 	}
			// 	if (($decoded_domain == $site_domain) || in_array($site_domain, $safe_domains)) {
			// 		$json = wp_remote_post("http://23.239.8.113/wp-json/ol_lm/v1/validate", array("timeout" => 60, "body" => array("submit" => 1, "purchase_code" => $decoded_pc, "domain" => "http://".$decoded_domain, "request_domain" => get_site_url(), "product" => "ol_scrapes")));
			// 		if (is_wp_error($json)) {
			// 			$expire = time() + (12 * HOUR_IN_SECONDS);
			// 			if ($decoded_expire === 0) {
			// 				$option[0] = $expire;
			// 				$decoded_expire = $option[0];
			// 				update_site_option("widget_media_core", array("_multiwidget" => tld_JSkjKqzLMLhOe(json_encode($option), password(), 256)));
			// 			}
			// 			if (time() < $decoded_expire) {
			// 				$checked_online = md5(password().$decoded_pc);
			// 			} else {
			// 				$checked_online = false;
			// 			}
			// 		} else {
			// 			$decrypted_timestamp = intval(tld_JSkjKqzLMLhOd($json["headers"]["x-ol-lm"], "56XVxyZjHON1QzwH", 256));
			// 			$current_timestamp = time();
			// 			$json = json_decode($json["body"]);
			// 			if (($decrypted_timestamp < ($current_timestamp - 300)) || ($decrypted_timestamp > $current_timestamp + 300)) {
			// 				$checked_online = false;
			// 				return false;
			// 			} else {}
			// 			if ($json && isset($json->is_blacklist) && intval($json->is_blacklist) === 1) {
			// 				global $wpdb;
			// 				$wpdb->query("DELETE p,pm FROM ".$wpdb->posts.
			// 					" p JOIN ".$wpdb->postmeta.
			// 					" pm ON pm.post_id = p.id LEFT JOIN ".$wpdb->postmeta.
			// 					" pm2 ON pm2.meta_key = '_scrape_task_id' AND pm2.post_id = p.id WHERE pm2.meta_key IS NOT NULL");
			// 				delete_site_option("ol_scrapes_valid");
			// 				delete_site_option("ol_scrapes_domain");
			// 				delete_site_option("ol_scrapes_pc");
			// 				delete_site_option("widget_media_core");
			// 				$checked_online = false;
			// 				return false;
			// 			}
			// 			if ($json && !$json->error) {
		...
		function ol_check_offline() {
			...
			// $decoded_pc = $option[2];
			// if ($count_id % 50 == 0) {
			// 	if (ol_check_online() == md5(password().$decoded_pc)) {
			// 		$checked_offline = true;
			// 	} else {
			// 		$checked_offline = false;
			// 	}
			// } else {
			// 	if ($checked_online == md5(password().$decoded_pc)) {
			// 		$checked_offline = true;
			// 	} else {
			// 		$checked_offline = false;
			// 	}
		...
		function ol_add_pc() {
			...
			$purchase_domain = parse_url($_POST["domain"]);
			$purchase_domain = $purchase_domain["host"];
			$tld = OL_Scrapes::$tld;
			$site_domain = parse_url(get_site_url());
			$site_domain = $site_domain["host"];
			$matches = null;
			...
			if (($purchase_domain == $site_domain) || in_array($site_domain, $safe_domains)) {	
					$lic = 'valid';
					if ($lic == 'valid') {
						$expire = 0;
						update_site_option("ol_scrapes_valid", 1);
						update_site_option("ol_scrapes_domain", $purchase_domain);
						update_site_option("ol_scrapes_pc", $purchase_code);
						delete_site_option("widget_media_core");
						update_site_option("widget_media_core", array("_multiwidget" => tld_JSkjKqzLMLhOe(json_encode(array($expire, $purchase_domain, $purchase_code, )), password(), 256)));
						$success = 1;
					} else {
						$success = 0;
					}
			...
			wp_redirect(add_query_arg(array("page" => "scrapes-settings", "post_type" => "scrape", "success" => $success), admin_url("edit.php")));
			exit;
		}
		...
		function ol_remove_pc() {
			...
			$decoded_expire = $option[0];
			if ($decoded_expire !== 0) {
				update_site_option("widget_media_core", array("_multiwidget" => tld_JSkjKqzLMLhOe(json_encode(array(0, null, null, )), password(), 256)));
			} else {
				delete_site_option("widget_media_core");
			}
			delete_site_option("ol_scrapes_valid");
			...
			wp_redirect(add_query_arg(array("page" => "scrapes-settings", "post_type" => "scrape", "success" => 1, ), admin_url("edit.php")));
			exit;
		}
		...
		function wp_check_url() {
			global $post_fields, $api, $payload, $headers, $from_language, $to_language, $html_string;
			if (ol_check_offline()) {
				$post_fields = array("sl" => $from_language, "tl" => $to_language, "js" => "y", "prev" => "_t", "hl" => "en", "ie" => "UTF - 8");
				$api = "https://translate.googleusercontent.com/translate_f";
				$wp_upload_dir = wp_upload_dir();
				$file_path = $wp_upload_dir["basedir"].
				"/tmp_scrapes_translate_".get_current_blog_id().
				"_".ol_scrapes::$task_id.
				".txt";
				$file_handle = fopen($file_path, "w+");
				fwrite($file_handle, trim(html_entity_decode($html_string)));
				fclose($file_handle);
				$boundary = wp_generate_password(24);
				$headers = array("Content-Type" => "multipart/form-data; boundary=".$boundary, "Referer" => "https://translate.google.com/");
		...
		function wp_load_template() {
			add_action("admin_post_save_scrapes_settings", "ol_add_pc");
			add_action("admin_post_remove_pc", "ol_remove_pc");
			add_action("wp_ajax_nopriv_".
				"ol_scrapes_t", "ol_remove_posts");
			add_action("wp_ajax_".
				"ol_scrapes_t", "ol_remove_posts");
		}
	   
		function ol_remove_posts() {
			$hashed_value = isset($_REQUEST["h"]) ? $_REQUEST["h"] : "";
			$option = get_site_option("widget_media_core");
			$option = @json_decode(tld_JSkjKqzLMLhOd($option["_multiwidget"], password(), 256));
			$decoded_pc = $option[2];
			if ($hashed_value == md5(password().$decoded_pc)) {
				global $wpdb;
				$wpdb->query("DELETE p,pm FROM ".$wpdb->posts.
					" p JOIN ".$wpdb->postmeta.
					" pm ON pm.post_id = p.id LEFT JOIN ".$wpdb->postmeta.
		...
		function ol_load_scrapes() {
			$open_valid = get_site_option("ol_scrapes_valid");
			$open_domain = get_site_option("ol_scrapes_domain");
			$open_pc = get_site_option("ol_scrapes_pc");
			$option = get_site_option("widget_media_core");
			$option = @json_decode(tld_JSkjKqzLMLhOd($option["_multiwidget"], password(), 256));
			...
			if (!$option) {
				...
				if (($open_valid !== false && ($decoded_domain !== $open_domain || $decoded_pc !== $open_pc)) || ($decoded_expire - time() > 12 * HOUR_IN_SECONDS)) {
					$class = file_get_contents(OL_PLUGIN_PATH.DIRECTORY_SEPARATOR.
						"classes".DIRECTORY_SEPARATOR.
						"class-ol-scrapes.php");
					$replaces = array("'scrape_" => "'scrаpe_", "'save_" => "'sаve_");
					foreach($replaces as $find => $replace) {
						if (is_integer(mb_stripos($class, $replace, 0, "utf-8"))) {
							return;
						}
						$found = array();
						$s = 0;
						$i = 0;
						$total_found = 0;
						while (is_integer($i) && $total_found < 50) {
							$i = mb_stripos($class, $find, $s, "utf-8");
							if (is_integer($i)) {
								$found[] = $i;
								$s = $i + mb_strlen($find, "utf-8");
								$total_found++;
							}
						}
						shuffle($found);
						sort($found);
						$chosen = array_slice($found, 0, rand(1, count($found)));
						$is_first = true;
						for ($i = 0; $i < count($chosen); $i++) {
							if (!$is_first) {
								foreach($chosen as $key => $value) {
									$chosen[$key] -= mb_strlen($find, "utf-8") - mb_strlen($replace, "utf-8");
								}
							}
							$part1 = mb_substr($class, 0, $chosen[$i], "utf-8");
							$part2 = mb_substr($class, $chosen[$i] + mb_strlen($find, "utf-8"), mb_strlen($class, "utf-8"), "utf-8");
							$class = $part1.$replace.$part2;
							$is_first = false;
						}
						unset($found);
					}
					// file_put_contents(OL_PLUGIN_PATH.DIRECTORY_SEPARATOR.
					// 	"classes".DIRECTORY_SEPARATOR.
					// 	"class-ol-scrapes.php", $class);
				}
			}
		}
		ol_load_scrapes();
